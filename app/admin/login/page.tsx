...
const isDemoMode = process.env.NEXT_PUBLIC_DEMO_MODE === "true" || !process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
      ...
      <form className="card w-full space-y-5 p-6" onSubmit={onSubmit}>
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {isDemoMode ? "Demo mode active. Enter admin preview without authentication." : "Use your Supabase-authenticated admin credentials."}
          </p>
        </div>
        ...
        {!isDemoMode ? (
          ...
          <div className="rounded-lg border border-amber-300/30 bg-amber-500/10 p-3 text-sm text-amber-700">
            Backend services are not configured. You can still preview admin UI and public catalog.
          </div>
        )}
        ...
        <button className="btn-primary w-full" disabled={pending}>
          {isDemoMode ? "Enter demo admin" : pending ? "Signing in..." : "Sign in"}
        </button>
      </form>
    </main>
  );
