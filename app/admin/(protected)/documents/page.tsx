...
export default async function DocumentsPage() {
  ...
  const { data: documentsData, error } = await supabase
    .from("documents")
    .select("id, title, file_size, created_at, visibility, show_in_catalog, public_slug")
    .eq("organization_id", ctx.organizationId)
    .order("created_at", { ascending: false });

  if (error && !isDemoMode()) throw new Error(error.message);

  const documents = isDemoMode() ? demoDocuments : documentsData ?? [];
          ...
          <tbody>
            {documents.map((document) => (
              <tr key={document.id} className="border-b border-border last:border-b-0">
                ...
                <td className="px-4 py-3 text-muted-foreground">{document.visibility}{document.show_in_catalog ? " • catalog" : ""}</td>
