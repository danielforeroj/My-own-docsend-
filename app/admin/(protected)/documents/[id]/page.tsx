...
export default async function DocumentDetailPage({ params }: { params: { id: string } }) {
  ...
  if (isDemoMode()) {
    const demoDoc = demoDocuments.find((doc) => doc.id === params.id) ?? demoDocuments[0];
    return (
      <div className="space-y-6">
        <Link href="/admin/documents" className="text-sm text-muted-foreground hover:text-foreground">← Back to documents</Link>
        <h1 className="text-3xl font-semibold tracking-tight">{demoDoc.title} <span className="text-sm text-amber-600">(demo)</span></h1>
        <section className="card p-5 text-sm text-muted-foreground">Connect Supabase to edit persistent landing and visibility settings for this document.</section>
      </div>
    );
  }
  ...
  const { data: document } = await supabase
    .from("documents")
    .select("id, title, created_at, storage_path, landing_page, visibility, public_slug, show_in_catalog")
    .eq("id", params.id)
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="card p-5">
        <h2 className="mb-3 text-lg font-semibold">Visibility</h2>
        <form action={visibilityAction} className="grid gap-4 md:grid-cols-2">
          <div className="space-y-1">
            <label className="label">Visibility</label>
            <select name="visibility" defaultValue={document.visibility ?? "private"} className="w-full">
          ...
          <div className="space-y-1">
            ...
            <input name="public_slug" defaultValue={document.public_slug ?? ""} className="w-full" />
          </div>
          <label className="flex items-center gap-2 text-sm md:col-span-2">
            <input type="checkbox" name="show_in_catalog" defaultChecked={Boolean(document.show_in_catalog)} /> Show in public catalog
          </label>
