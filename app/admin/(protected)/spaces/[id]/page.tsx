...
export default async function SpaceDetailPage({ params }: { params: { id: string } }) {
  ...
  if (isDemoMode()) {
    const demoSpace = demoSpaces.find((space) => space.id === params.id) ?? demoSpaces[0];
    return (
      <div className="space-y-6">
        <Link href="/admin/spaces" className="text-sm text-muted-foreground hover:text-foreground">← Back to spaces</Link>
        <h1 className="text-3xl font-semibold tracking-tight">{demoSpace.name} <span className="text-sm text-amber-600">(demo)</span></h1>
        <section className="card p-5 text-sm text-muted-foreground">Connect Supabase to edit persistent landing and visibility settings for this space.</section>
      </div>
    );
  }
  ...
  const { data: space } = await supabase
    .from("spaces")
    .select("id, name, description, created_at, is_active, landing_page, visibility, public_slug, show_in_catalog")
    .eq("id", params.id)
      ...
      <section className="card p-5">
        ...
        <form action={visibilityAction} className="grid gap-4 md:grid-cols-2">
          <div className="space-y-1">
            ...
            <select name="visibility" defaultValue={space.visibility ?? "private"} className="w-full">
          ...
          <div className="space-y-1">
            ...
            <input name="public_slug" defaultValue={space.public_slug ?? ""} className="w-full" />
          </div>
          <label className="flex items-center gap-2 text-sm md:col-span-2">
            <input type="checkbox" name="show_in_catalog" defaultChecked={Boolean(space.show_in_catalog)} /> Show in public catalog
          </label>
