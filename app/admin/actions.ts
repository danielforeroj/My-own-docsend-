...
export async function createSpace(formData: FormData) {
  ...
  const { data: space, error: spaceError } = await supabase
    ...
    .insert({
      ...
      visibility,
      public_slug: visibility === "public" ? (publicSlugRaw ? slugify(publicSlugRaw) : slugify(name)) : null,
...
export async function updateDocumentVisibility(documentId: string, formData: FormData) {
  ...
  const { error } = await supabase
    ...
    .update({
      visibility,
      public_slug: visibility === "public" ? (publicSlugRaw ? slugify(publicSlugRaw) : null) : null,
      show_in_catalog: visibility === "public" ? showInCatalog : false
    })
    .eq("id", documentId)
    .eq("organization_id", ctx.organizationId);
